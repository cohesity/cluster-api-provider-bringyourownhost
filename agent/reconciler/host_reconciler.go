...
const (
	...
	// ControlPlaneUpgradePreflightErrors are the preflight errors to ignore during control plane upgrade
	ControlPlaneUpgradePreflightErrors = "CoreDNSUnsupportedPlugins,CoreDNSMigration,CreateJob"
...
func (r *HostReconciler) reconcileNormal(ctx context.Context, byoHost *infrastructurev1beta1.ByoHost) (ctrl.Result, error) {
	...
	if !conditions.IsTrue(byoHost, infrastructurev1beta1.K8sNodeBootstrapSucceeded) {
		...
		if metav1.HasAnnotation(byoHost.ObjectMeta, infrastructurev1beta1.K8sNodeBootstrappedAnnotation) {
			...
			byoHost.Annotations[infrastructurev1beta1.K8sNodeBootstrappedAnnotation] = StatusTrue
			logger.Info("Node bootstrap completed, annotation set")
		}
...
func (r *HostReconciler) setBootstrapSuccessAnnotation(ctx context.Context, byoHost *infrastructurev1beta1.ByoHost) {
	...
	if !conditions.IsTrue(byoHost, infrastructurev1beta1.K8sNodeBootstrapSucceeded) {
		logger.Info("K8sNodeBootstrapSucceeded condition is not true, skipping annotation")
		return
	}
	...
	byoHost.Annotations[infrastructurev1beta1.K8sNodeBootstrappedAnnotation] = StatusTrue
	logger.Info("Bootstrap success annotation set to true")
}
...
func (r *HostReconciler) restartKubelet(ctx context.Context) error {
	...
	if err := r.CmdRunner.RunCmd(kubeletCtx, restartKubeletCmd); err != nil {
